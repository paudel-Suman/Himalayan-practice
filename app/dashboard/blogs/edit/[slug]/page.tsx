...
const BlogEditPage = () => {
    ...
    const fetchSingleBlog = async () => {
      try {
        ...
        setFormData({
          ...
          image: (blogData.image && setBlogImage(blogData.image)) || "",
        });
      } catch (error) {
        console.error(error);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_SERVER_API}/blog/update-blog/${blogId}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <main className="my-6">
      ...
      <h2 className="text-xl font-bold mt-10">Edit Blogs</h2>
