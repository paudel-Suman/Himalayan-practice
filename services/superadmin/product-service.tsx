...
export class productService {
  static async fetchAllActiveProducts(page: number): Promise<ProductResponse> {
    const token = Cookies.get("token");
    if (!token) {
      throw new Error(MESSAGES.TOKEN.TOKEN_NOT_FOUND);
    }
    return await apiClient<ProductResponse>(
      `${API_ROUTES.SUPERADMIN_ADMIN.FETCH_ALL_ACTIVE_PRODUCT}?page=${page}`,
      {
        method: "GET",
        token,
      }
  ...
  static async fetchAllInactiveProducts(page: number): Promise<ProductResponse> {
    const token = Cookies.get("token");
    if (!token) {
      throw new Error(MESSAGES.TOKEN.TOKEN_NOT_FOUND);
    }
    return await apiClient<ProductResponse>(
      `${API_ROUTES.SUPERADMIN_ADMIN.FETCH_ALL_DELETE_PRODUCT}?page=${page}`,
      {
        method: "GET",
        token,
      }
